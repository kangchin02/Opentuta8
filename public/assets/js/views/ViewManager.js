define(function () {
    ...
    var MobileViewManager = ViewManager.extend({
        ...
        _showProgress : function(title, label){
            ...
            if ( (title == null) != (label == null) ) {
                ...
                var loaderTemplate = _.template('<h1><%= title %></h1>' +
                    '<div class="center"><p><%= label %></p><img src="/css/images/ajax-loader.gif"/></div>');

                loaderOptions = {
                    theme: theme,
                    textVisible: true,
                    html: loaderTemplate({ title: title, label: label })
                };

            }

            var $loader = $.mobile.loading( 'show', loaderOptions);
    ...
    if ($$m.v.viewManager != null) {
        alert("View Manager Instance already exists!");
    }

    $$m.v.viewManager = viewutils.isMobileLayout() ? new MobileViewManager() : new ViewManager;

    return $$m.v.viewManager;
});
